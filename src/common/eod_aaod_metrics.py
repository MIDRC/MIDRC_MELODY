...
def print_table_of_nonzero_eod_aaod(eod_aaod: Dict[str, Dict[str, Dict[Any, Dict[str, Tuple[float, Tuple[float, float]]]]]],
    ...
    """
    Print three tables:
      1. All EOD median values (with rows colored if criteria are met: median > 0.1 or < -0.1).
      2. All AAOD median values (with rows colored if median > 0.1).
            ...
            for group, metrics in groups.items():
                for metric in ['eod', 'aaod']:
                    if metric not in metrics:
                        continue
                    median_val, (ci_lower, ci_upper) = metrics[metric]
                    # Determine if criteria are met and choose color if necessary.
                    if metric == 'eod':
                        qualifies = (median_val > 0.1 or median_val < -0.1)
                    ...
                    upper_str = f"{color}{ci_upper:.4f}{reset}" if qualifies else f"{ci_upper:.4f}"
                    # Build table rows for all values.
                    row = [model, category, group, median_str, lower_str, upper_str]
                    if metric == 'eod':
                        all_eod.append(row)
                    elif metric == 'aaod':
                        all_aaod.append(row)
                    # Build filtered rows (always colored) if criteria are met.
                    if qualifies:
                        filtered.append([
                            model,
                            category,
    ...
    if filtered:
        print("EOD/AAOD median values meeting criteria:")
